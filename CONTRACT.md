...
`anomalous_failed_scans_24h` (v3.5, docs/urgent_scan_corruption.md Step 4) — `COUNT(*)` over `scans` in the last 24h where `status = 'failed'` and `overall_grade IS NOT NULL`. A failed scan never has a grade through any normal path (`_mark_failed` never sets `overall_grade`/`overall_score`/`headline`); the only way this combination occurs is a scan completing successfully and then being overwritten as failed afterward — the shape Finding 4 shipped. Should always be `0`. The frontend renders it in red, same treatment as `monitors_overdue_1h`, whenever it's non-zero.
